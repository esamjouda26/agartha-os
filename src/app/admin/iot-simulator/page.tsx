...
export default function IoTSimulatorPage() {
  ...
  const DeviceSelect = ({ value, onChange, id }: { value: string; onChange: (v: string) => void; id: string }) => (
    fetchingDevices ? (
      <div className="h-[38px] bg-[#020408] border border-white/10 rounded-md flex items-center px-3 text-xs text-gray-500 animate-pulse">Loading…</div>
    ) : (
      <select id={id} value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-[#020408] border border-white/10 rounded-md px-3 py-2.5 text-sm focus:border-[rgba(212,175,55,0.5)] focus:outline-none" style={{ colorScheme: "dark" }}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="font-cinzel text-xl font-bold text-[#d4af37] tracking-wider">IoT Hardware Simulator</h1>
          <p className="text-xs text-gray-500 mt-1 uppercase tracking-widest">
            Spoof physical scans — turnstiles &amp; badge readers
          </p>
        </div>
        <div className="flex items-center gap-2 bg-yellow-500/5 border border-yellow-500/20 rounded-full px-4 py-1.5">
          <span className="text-[10px] text-yellow-400 tracking-widest uppercase font-bold">⚠ IT Admin Tool</span>
        </div>
      </div>

      {/* PSK Config */}
      <div className="glass-panel rounded-lg p-4 border-yellow-500/20">
        <div className="flex items-start gap-3">
          <span className="text-yellow-400 text-lg">🔐</span>
          <div className="flex-1">
            <h4 className="text-xs font-bold text-yellow-400 uppercase tracking-widest mb-1">Webhook Secret</h4>
            <p className="text-[11px] text-gray-400 leading-relaxed mb-2">
              Paste <code className="text-[#d4af37] bg-[rgba(212,175,55,0.1)] px-1.5 py-0.5 rounded text-[10px] font-mono">IOT_WEBHOOK_SECRET</code> — used for both turnstile and crew-scan endpoints.
            </p>
            <input
              type="password"
              ...
              className="w-full max-w-md bg-[#020408] border border-white/10 rounded-md px-3 py-2 text-xs font-mono focus:border-[rgba(212,175,55,0.5)] focus:outline-none"
            />
          </div>
        </div>
      </div>
      ...
      {/* Tab Selector */}
      <div className="flex gap-1 bg-[#020408] border border-white/10 rounded-lg p-1 w-fit">
        {[
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md text-xs font-bold uppercase tracking-wider transition-all ${activeTab === tab.key ? "bg-[rgba(212,175,55,0.2)] text-[#d4af37] border border-[rgba(212,175,55,0.4)]" : "text-gray-500 hover:text-gray-200"}`}
          >
            {tab.label}
          </button>
        ))}
      ...
      <div className="glass-panel rounded-lg p-6">
        {activeTab === "turnstile" ? (
          <>
            <h3 className="text-sm font-bold text-[#d4af37] uppercase tracking-widest mb-4">Simulate Turnstile Scan</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                <label htmlFor="qr-input" className="block text-[10px] text-gray-500 uppercase tracking-widest mb-1">QR Code Reference</label>
                <input
                  ...
                  className="w-full bg-[#020408] border border-white/10 rounded-md px-3 py-2.5 text-sm font-mono focus:border-[rgba(212,175,55,0.5)] focus:outline-none"
                />
              </div>
              <div>
                <label htmlFor="turnstile-device" className="block text-[10px] text-gray-500 uppercase tracking-widest mb-1">Device Serial</label>
              ...
              <div className="flex items-end">
                <button onClick={handleTurnstileScan} disabled={turnstileLoading || !qrCode.trim()}
                  className="w-full bg-gradient-to-r from-[#806b45] to-[#d4af37] hover:from-[#d4af37] hover:to-yellow-300 text-[#020408] font-bold py-2.5 px-6 rounded-md transition-all text-xs uppercase tracking-widest disabled:opacity-40 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(212,175,55,0.2)]">
          ...
          <>
            <h3 className="text-sm font-bold text-[#806b45] uppercase tracking-widest mb-4">Simulate Crew Badge Scan</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                <label htmlFor="emp-input" className="block text-[10px] text-gray-500 uppercase tracking-widest mb-1">Employee ID</label>
                <input
              ...
              <div>
                <label htmlFor="badge-device" className="block text-[10px] text-gray-500 uppercase tracking-widest mb-1">Badge Reader</label>
              ...
              <div className="flex items-end">
                <button onClick={handleCrewBadgeScan} disabled={badgeLoading || !employeeId.trim()}
                  className="w-full bg-[#020408] border border-[rgba(128,107,69,0.5)] text-[#806b45] hover:border-[rgba(128,107,69,0.8)] hover:bg-[rgba(128,107,69,0.1)] font-bold py-2.5 px-6 rounded-md transition-all text-xs uppercase tracking-widest disabled:opacity-40 disabled:cursor-not-allowed">
      ...
      {/* Response Log */}
      <div className="glass-panel rounded-lg overflow-hidden border-red-500/20">
        <div className="bg-[#020408]/80 border-b border-white/10 flex items-center justify-between px-5 py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            {logs.length > 0 && (
              <button onClick={() => setLogs([])} className="text-[10px] text-red-500/60 hover:text-red-400 uppercase tracking-widest transition">Clear</button>
            )}
          </div>
        </div>
        <div ref={logContainerRef} className="max-h-[400px] overflow-y-auto font-mono text-xs bg-[#010204]">
          {logs.length === 0 ? (
            ...
            logs.map((entry) => {
              ...
              return (
                <div key={entry.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                  <div className="px-5 py-3 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="text-[10px] text-gray-600 flex-shrink-0 w-16">
                        {new Date(entry.timestamp).toLocaleTimeString()}
                      </span>
                      <span>{typeLabel}</span>
                      <code className="text-[11px] text-[#d4af37] truncate">{entry.input}</code>
                      <span className="text-white/20">@</span>
                      <code className="text-[11px] text-gray-500 truncate">{entry.device_serial}</code>
                    </div>
                  ...
                  <div className="px-5 pb-3">
                    <pre className="text-[10px] text-gray-500 bg-[#020408]/80 border border-white/5 rounded p-3 overflow-x-auto whitespace-pre-wrap">
